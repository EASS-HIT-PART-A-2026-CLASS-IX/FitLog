...
def demo():
    ...
    workouts = [
        {"exercise": exercises[0], "date": "2026-03-23", "sets": 4, "reps": 6, "weight": 120.0},
        {"exercise": exercises[1], "date": "2026-03-23", "sets": 3, "reps": 8, "weight": 90.0},
        {"exercise": exercises[2], "date": "2026-03-24", "sets": 3, "reps": 5, "weight": 150.0},
        {"exercise": exercises[3], "date": "2026-03-25", "sets": 3, "reps": 8, "weight": 70.0},
    ]
    
    for w in workouts:
        log = client.post("/logs/", json={
        ...
        print(f"   ✅ {w['date']}: {w['sets']}×{w['reps']} @ {w['weight']} kg ({w['exercise']['name']})")
    print()
    
    # 5. Log Macros
    print("🥗 Step 5: Logging Daily Nutrition...")
    macros = [
        {"date": "2026-03-23", "calories": 3000, "protein": 180, "carbs": 350, "fat": 80},
        {"date": "2026-03-24", "calories": 2800, "protein": 175, "carbs": 300, "fat": 85},
        {"date": "2026-03-25", "calories": 2300, "protein": 170, "carbs": 250, "fat": 70},
    ]
    
    for m in macros:
        macro = client.post("/macros/", json={
            "entry_date": m["date"],
            "calories": m["calories"],
            "protein_g": m["protein"],
            "carbs_g": m["carbs"],
        ...
        print(f"   ✅ {m['date']}: {m['calories']} kcal | P:{m['protein']}g C:{m['carbs']}g F:{m['fat']}g")
    print()
    ...
    try:
        ai_response = client.post("/ai/chat", json={
            "user_id": profile_id,
